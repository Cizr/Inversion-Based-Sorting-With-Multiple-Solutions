...
class Reverter:
    ...
    def hash(self):
        """Computes a hashcode of the array."""
        ...
        while not current_state.is_the_goal():
            ...
            for action in actions:
                if action.__hash__ not in visited:
                    # Cost function g is the depth of the current state
                    cost = action.depth() + action.heuristic()
                    next_states.append((action, cost))
            if not next_states:
                return None
            next_states.sort(key=lambda x: x[1])
            current_state, _ = next_states[0]
            visited.add(current_state.__hash__)
    ...
    def depth(self) -> int:
        """Calculates the depth of the current state in the search tree."""
        depth = 0
